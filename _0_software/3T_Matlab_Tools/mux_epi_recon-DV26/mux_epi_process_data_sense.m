...
use_previous_smap = (isfield(p, 'smap') && ~isempty(p.smap)) && ~(p.RT_flag && p.cal_flag); % True: Use previously saved sensitivity maps; False: will calculate sensitivity maps from calibration data
...
if p.num_mux_cycle > 0 && p.cal_flag
...
if use_previous_smap
    ...
    if p.calc_snr_pmr || p.calc_sense_rsnr || (p.RT_flag && p.cal_flag)
...
if p.num_mux_cycle > 0 && p.cal_flag
    ...
    % Use SENSE
    if ~p.return_sos_im
        [dat_cal, p] = sense_coil_comb(dat_cal, p); % Output dat_cal Dim: [X(=nx), Y(=p.ny_pres), Echo(=nec), Slice(=nsl*nz, z indices -floor(nz/2):1:(ceil(nz/2)-1)), Coil(=1), Time(=p.num_mux_cycle)]. Input dat_cal Dim: [Kx(=nx), Ky(=nky_input), Echo(=nec), Slice(=nsl*nz, z indices -floor(nz/2):1:(ceil(nz/2)-1)), Coil(=nc), Time(=p.num_mux_cycle)].
    end
    ...
    if p.partial_ky
        ...
        dat_cal = homodyne(dat_cal, ny_part, p.homodyne_ntran, p.homodyne_niter, datin_domain, 'xy', p.use_GPU); % Dim: [X(=nx), Y(=p.ny_pres), Echo(=nec), Slice(=nsl*nz, z indices -floor(nz/2):1:(ceil(nz/2)-1)), Coil(=1 if ~p.return_sos_im; =nc if p.return_sos_im), Time(=p.num_mux_cycle)]
    else
        if p.return_sos_im
...
if size(dat, p.T_DIM) > p.mux_encoded*p.num_mux_cycle || ~p.cal_flag
    ...
    if ~isfield(p, 'us_msk') || ~isempty(p.us_msk)
        if p.calc_snr_pmr && isfield(p, 'pmr_us_msk') && ~isempty(p.pmr_us_msk)
            p.us_msk = p.pmr_us_msk;
        else
            p.us_msk = get_ky_omegaz_us_msk(p, sz_dat.y, size(dat, p.T_DIM) - p.mux_encoded*p.num_mux_cycle, true);
        end
    end
    
    % Data
    if p.cal_flag
        dat = dat(:, p.us_msk(1).ky, :, :, :, p.mux_encoded*p.num_mux_cycle+1:end); % dat contains accelerated data only. If partial ky was used, dat doesn't contain the unacquired k-space portion. Dim: [Kx, Ky(=nsamp), Echo, Slice, Coil, Time(accelerated time points)]
    else
        dat = dat(:, p.us_msk(1).ky, :, :, :, :);
    end
    
    % SENSE
    if isfield(p, 'pha_coe')
        pha_coe = p.pha_coe(:, p.us_msk(1).ky, :, :, :, :);
    ...
    [dat, p, ranks] = mux_recon_sense(dat, p.us_msk, p, smap, eddy); % dat Dim: [X, Y, Echo, Slice*SimultaneousSlice, Coil(=1), Time, SetOfSensitivityMaps]
    ...
    if p.partial_ky
        ...
        dat = homodyne(dat, ny_part, p.homodyne_ntran, p.homodyne_niter, 'xy', 'xy', p.use_GPU); % Dim: [X, Y, Echo, Slice*SimultaneousSlice, Coil(=1), Time, SetOfSensitivityMaps]
        sz_dat.y = p.ny_pres;
    end
