...
if p.partial_ky
    dat_cal = cat(p.PE_DIM, dat_cal, zeros(sz.x, p.ny_pres-sz.y, sz.ec, sz.sl, sz.c, sz.nz, 'like',dat_cal)); % Dim: [Kx, Ky, Echo, Slice, Coil, SimultaneousSlice Z]
    sz.y = p.ny_pres;
end
...
if strcmp(p.smap_type, 'espirit') % ESPIRiT sensitivity maps
    ...
    if ~p.espirit_catim % Calculate sensitivity maps slice by slice
        ...
        smap = zeros(sz_smap(1), sz_smap(2), sz.c, p.espirit_nmap, sz.ec, sz.sl, sz.nz, 'like',dat_cal); % Dim: [X, Y, Coil, SetOfSensitivityMaps, Echo, Slice, SimultaneousSlice Z]
        for im_idx = 1 : sz.ec * sz.sl * sz.nz
            smap(:, :, :, :, im_idx) = sense_map_espirit((dat_cal(:, :, :, im_idx)), p.espirit_nmap, ncalib, ...
                p.espirit_ksize, p.espirit_eigThresh_1, p.espirit_eigThresh_2, p.partial_ky, ny_part, false, p.espirit_tmp_dir, p.use_GPU);
        end
        ...
        smap = zeros(sz_smap(1), sz_smap(2)*sz.nz, sz.c, p.espirit_nmap, sz.ec, sz.sl, 'like',dat_cal); % Dim: [X, Y->SimultaneousSliceZ, Coil, SetOfSensitivityMaps, Echo, Slice]
        for im_idx = 1 : sz.ec * sz.sl
            smap(:, :, :, :, im_idx) = sense_map_espirit((dat_cal(:, :, :, im_idx)), p.espirit_nmap, ncalib, ...
                p.espirit_ksize, p.espirit_eigThresh_1, p.espirit_eigThresh_2, false, [], false, p.espirit_tmp_dir, p.use_GPU);
        end
