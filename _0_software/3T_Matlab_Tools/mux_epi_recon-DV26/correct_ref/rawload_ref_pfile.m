...
p.frames       = frames;
p.coils        = coils;
p.epiecho      = hdr.rdb_hdr.user27;
if p.epiecho > 1                                                % Update the number of slices only when it's a multi-echo sequence
    p.num_slices   = p.num_slices * p.epiecho;
end
...
if ~p.rds_data_order                                            % Pfile saved by the scanner
    if hdr.rdb_hdr.ref == 5
        dat = rawloadX_tseries(refpfile, [], [], slices_in_pfile, [], 1, p); % Load the 1st pass of the pfile if integrated refscan
    else
