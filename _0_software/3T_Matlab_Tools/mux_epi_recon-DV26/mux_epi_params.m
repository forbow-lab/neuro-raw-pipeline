function p = mux_epi_params(pfile, slices, nt_to_recon, n_vcoils, debug, recon_method, apply_fermi, use_homodyne, notch_thresh, noise_fname, refp,...
                            RT_flag, GPU_flag, pmr_reps)
%
% p = mux_epi_params(pfile, [slices=[]], [nt_to_recon=[]], [n_vcoils=[]], [debug=false], [recon_method='1Dgrappa'], [apply_fermi=false], ...
%                    [use_homodyne=true], [notch_thresh=0], [noise_fname], [refp], [RT_flag=false], [GPU_flag=0], [pmr_reps=0])
%
...
%   refp        - Filename of reference scan pfile.
%   RT_flag     - True: real-time recon pipeline; False: non-realtime recon pipeline. Default: false
%   GPU_flag    - 1: use GPU; 0: not use GPU. Array (e.g. [1,2,3]): list of GPU devices to use. Default: 0
%   pmr_reps    - Number of repeats for pseudo multiple replica. Default: 0
%
% Output:
...
if(~exist('RT_flag', 'var'));                              RT_flag = false;     end
if(~exist('GPU_flag', 'var'));                             GPU_flag = 0;        end
if(~exist('pmr_reps', 'var'));                             pmr_reps = 0;        end

% Save the path to pfile
p.pfile = pfile;
...
if p.pfile_header.version >= 26 && hdr.rdb_hdr.mb_factor > 1 % If using Hyperband product sequence
    p.mux_excited        = hdr.rdb_hdr.mb_factor;            % Number of simultaneously excited slices
    p.num_slices         = ceil(hdr.image.slquant/p.mux_excited) + (p.mux_excited > 1) * (1 - mod(ceil(hdr.image.slquant/p.mux_excited),2)); % Number of slices in one band. In Hyperband this number is always odd, i.e. the smallest odd number which satisfies num_slices * mb_factor >= slquant
    p.num_unmuxed_slices = hdr.image.slquant;                % Number of un-muxed slices. TODO- when num_slices * mux > slquant??
    p.use_gzblips        = (p.mux_excited ~= 1);             % True: used Gz blips for slice encoding; False: no Gz blips, direct aliasing of all simultaneous slices
    p.num_mux_cycle      = 0;                                % Number of complete slice phase cycling at the beginning of the slice-multiplexed scan
    p.sldist             = 0;                                % Distance between two adjacent simultaneously excited slices, in mm
    p.cap_random_flag    = 0;                                % Type of Gz blip phase encoding. 0: CAIPI; 1: Bit-reversed MICA; 2: Random MICA.
else                                                         % If using CNI mux sequence
    p.num_slices         = hdr.image.slquant;                
    p.mux_excited        = hdr.rdb_hdr.user6;
    p.num_unmuxed_slices = p.num_slices * p.mux_excited;     
    p.use_gzblips        = hdr.rdb_hdr.user13; 
    p.num_mux_cycle      = hdr.rdb_hdr.user7;                
    p.sldist             = hdr.rdb_hdr.user8;                
    p.cap_random_flag    = hdr.rdb_hdr.user17;               
end
...
p.transpose       = hdr.rdb_hdr.transpose;                   % flag indicating whether to transpose the final image
p.rotation        = hdr.rdb_hdr.rotation;                    % Number of 90-degree rotations to apply to the final image
...
p.te              = hdr.rdb_hdr.te;                          % TE 
p.epiecho         = hdr.rdb_hdr.user27;                      % Number of echoes in multi-echo sequence

% MJM - update for dynamic phase correction
if hdr.rdb_hdr.extra_frames_bot > 0,
    p.dynamic_phase_corr = 1; % Dynamic phase correction
else
    p.dynamic_phase_corr = 0; % Static phase correction
end

if p.epiecho > 1                                             % If it's a multi-echo sequence
    p.num_slices  = p.num_slices * p.epiecho;                % Number of slices in the slice loop in pfile
    p.te2         = hdr.rdb_hdr.te2;                         % TE of the second echo in multi-echo sequence
    p.te3         = hdr.rdb_hdr.user28;                      % TE of the third echo in multi-echo sequence
end
...
if p.pfile_header.version >= 26 && hdr.rdb_hdr.mb_factor > 1     % TODO: when num_unmuxed_slices > slquant
    switch p.num_unmuxed_slices
        case hdr.rdb_hdr.nslices/hdr.rdb_hdr.npasses
            p.acq_order = 'interleaved';                         % Interleaved acquisition, hdr.data_acq_tab.pass_number is always 0.
             % The acquisition order of the prescribed slices
            if p.rds_data_order && p.epiecho>1
                %p.sl_acq_order = hdr.data_acq_tab.slice_in_pass(1 : p.num_slices);
                ec1 = (hdr.data_acq_tab.slice_in_pass(1 : p.num_slices/3)-1)*3+1;
                p.sl_acq_order = ec1;
                for i=1:p.epiecho-1
                    p.sl_acq_order = [p.sl_acq_order; ec1+i];
                end
            else
                p.sl_acq_order = hdr.data_acq_tab.slice_in_pass(1 : p.num_slices);
            end
...
if p.cap_random_flag ~= 0
    error('Must use CAIPI gz blips.');
end
...
if p.caipi || p.mica_perturbed_caipi || p.mica_poisson
    if p.pfile_header.version >= 26 && hdr.rdb_hdr.mb_factor > 1
        p.cap_fov_shift  = hdr.rdb_hdr.mb_slice_fov_shift_factor;  % CAIPI FOV shift for the accelerated time points. A positive integer.
        p.cap_blip_start = floor(p.cap_fov_shift/2);               % Starting index of the kz blips for the accelerated time points. 0~(abs(cap_fov_shift)-1) correspond to -kzmax~kzmax.
        p.cap_blip_inc   = p.inplane_R;                            % Increment of the kz blip index for adjacent acquired ky lines
    else
        ...
        p.cap_fov_shift  = hdr.rdb_hdr.user21;  
    end
...
if p.pfile_header.version >= 26
    p.ref_for_default_ecc = 'ref.h5';                            % Which data to use for the default eddy current correction. 'ref.h5': use ref.h5 file; 'ecc data': use ECC data collected by the mux sequence; 'ref pfile': use reference scan pfile.
else
    p.ref_for_default_ecc = 'ref.dat';
end
p.skip_ref_pass = 0;
if exist('refp', 'var') && ~isempty(refp) && exist(refp, 'file')
    p.ref_for_default_ecc = 'ref pfile';
end
if (hdr.rdb_hdr.ref == 5)                                    % refscan k-space raw data saved in the first pass of pfile
    p.ref_for_default_ecc = 'ref_in_scan';
    p.skip_ref_pass = 1;
end
if p.md_ecc
...
p.cal_dat_tpoints    = p.skip_ref_pass + (p.mux*(p.num_mux_cycle-1)+1 : p.mux*p.num_mux_cycle); % Time points corresponding to the calibration data, i.e. the last group of mux phase cycling
p.zpad_image         = 0;                                    % True: Zero pad reconstructed images to size p.zpad_size
if p.zpad_image
    p.zpad_size      = [256, 256];                           % Matrix size to pad the reconstructed images to
end

% -- Specify reconstruction algorithm to use
if ~isempty(recon_method) && ~ischar(recon_method)
...
if isempty(recon_method)
    if p.isdifscan
        recon_method = '1Dgrappa_sense1';
    else
        recon_method = '1Dgrappa';
    end
end
...
if p.isdifscan
    p.fermi_rad_scale = 0.9;                            % Aapply a more aggressive fermi filter for diffusion scans
...
if ~p.return_sos_im && sum(strcmp(p.mux_recon_method, {'1Dgrappa', 'slice-grappa', 'split-slice-grappa'})) > 0 % If using GRAPPA
...
if strcmp(p.mux_recon_method, 'sense') && ~p.return_sos_im
    warning('SENSE recon cannot be used with sense1 coil combination. Set to SOS coil combination.\n');
    p.return_sos_im = 1;
    p.sense_coil_comb = 0;
end
...
if ~strcmp(p.ref_for_default_ecc, 'ref.h5') || p.md_ecc || (ismember(p.mux_recon_method, {'slice-grappa', 'split-slice-grappa'}) && p.slice_grappa_odd_even_fit)
...
p.cal_flag = 1;                                              % True: Reconstruct for the 1st time, need to claculate calibration-related quantities; False: Don't need to calibrate, used previously saved values. If not real-time recon, set this to true.
p.RT_flag = RT_flag;                                         % True: real-time recon pipeline; false: non-realtime pipeline
% Using GPU device
if (GPU_flag(1) ~= '0') && p.RT_flag && gpuDeviceCount > 0                           % Use GPU only for real-time recon
    if GPU_flag(1) == '1'
        dev = mod(slices, gpuDeviceCount);
        if dev==0
            dev = gpuDeviceCount;
        end
        gpuDevice(dev);
    elseif GPU_flag(1) == '['
        gpuDevice_inUse = str2num(GPU_flag);
        gpuDeviceCount_inUse = length(gpuDevice_inUse);
        dev_idx = mod(slices, gpuDeviceCount_inUse);
        if dev_idx==0
            dev_idx = gpuDeviceCount_inUse;
        end
        gpuDevice(gpuDevice_inUse(dev_idx));
    end
    p.use_GPU = 1;
    p.precision = 'single';
else
...
% -- For SNR calculation; Noise related parameters
% Specify SNR map params
if ~pmr_reps
    p.calc_snr_pmr = 0;                                      % True: calculate SNR maps using pseudo-multiple replica method.
else
    p.calc_snr_pmr = 1;
    p.pmr_num_rep = pmr_reps;                                % Number of replicas to simulate using the pseudo-multiple replica method.
...
if p.calc_snr_pmr
    p.sim_leakage = 0;                                       % True: Simulate signal leakage, i.e. use data from all slices in calibration and use data from slices specified in p.sim_leakage_keep_slices to synthesize accelerated data.
    p.sim_leakage_keep_slices = 1;                           % Slices to keep for synthesizing accelerated data when simulating signal leakage. Only used if p.sim_leakage is true.
end
if strcmp(p.mux_recon_method, 'sense')
    p.calc_sense_rsnr = 0;                                   % True: Calculate retained SNR and noise maps basing on the hybrid-space SENSE reconstruction model
    if p.mux_excited == 1
        p.calc_sense_rsnr = 0;
    end
...
if (p.calc_snr_pmr || (strcmp(p.mux_recon_method, 'sense') && p.calc_sense_rsnr)) && exist('noise_fname', 'var') && ~isempty(noise_fname) && exist(noise_fname, 'file')
    ...
    if strcmp(ext, '.dat')
        ...
        if p.isdifscan % muxarcepi2 sequence
            p.psi_mtx = p.psi_mtx .* 13.8;
        else % muxarcepi sequence
            p.psi_mtx = p.psi_mtx ./ 10^8;
        end
    elseif strcmp(ext, '.7')
        p_noise = p;
        noise = load_raw_tseries(noise_fname, p_noise, slices, 1:p.mux*p.num_mux_cycle);
        noise = reshape(permute(noise, [1,2,3,4,6,5]), [], p.num_coils);
        p.psi_mtx = calc_coil_noise_cov(noise, 2);
    end
    p.whiten_type = 'psi_mtx';          % 'psi_mtx': Use (p.psi_mtx)^(-1/2); 'coil_noise_std'(This option is needed because many scans don't measure psi_mtx): Use diag(1./p.coil_noise_std); 'none': don't conduct data whitening
else
    p.whiten_type = 'coil_noise_std';
end
...
if ~isfield(p, 'psi_mtx') || isempty(p.psi_mtx)
    if p.calc_snr_pmr
        ...
        warning('No coil covariance matrix information, cannot run pseudo multiple replica simulation.\n');
    end
end
