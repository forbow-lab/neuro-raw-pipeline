...
% -- Load k-space data    
% Try to read the pfile a few times until we are sure it's really an aborted scan, to deal with pfile reading hit the end of the file while it's still being written
if p.cal_flag
    EOF_threshold = 50;                                                 % Allow more trials to read all the calibration time points
else
    EOF_threshold = 10;
end
EOF_wait_secs = 0.5;
...
if isempty(dat)                                                         % Return empty dat, dat_ecc if reach the end of the pfile enough times and not able to read the correct data
    dat_ecc = [];
    return;
end
...
% -- Save the preprocessed raw data for slice-grappa recon
if p.RT_flag && p.cal_flag && (strcmp(p.mux_recon_method, 'slice-grappa') || strcmp(p.mux_recon_method, 'split-slice-grappa'))  % slice-grappa recon needs raw data to synthesize muxed data point for kernel fitting
    p.mux_raw_data = dat(:, :, :, :, :, p.mux_encoded * (p.num_mux_cycle-1) + 1 : p.mux_encoded * p.num_mux_cycle);
end
