function [dat_out, p, ranks] = mux_recon_sense(dat_mux, us_msk, p, smap, eddy)
%
...
use_previous_solvemtx = isfield(p, 'solve_mtx') && ~isempty(p.solve_mtx) && ...
                        ~(p.RT_flag && p.cal_flag) && ...
                        ~(~p.internal_cal && (p.tpoints_to_load == p.first_scan_pass));    % True: solve_mtx is already saved in p in previous passes, only used in real-time recon
...
dat_mux = ifftc(dat_mux, p.FE_DIM);                 % Slice multiplexed data in x-ky-omegaz space. Dim: [X(=nx), Ky(=nsamp), Echo(=nec), Slice(=nsl), Coil(=nc), Time(=nt)]
...
dat_out = zeros(datsz.x, ny, datsz.ec, datsz.sl, nz, nc_rec, nt, nmap, 'like',dat_mux); % Dim: [X(=nx), Y(=ny), Echo(=nec), Slice(=nsl), SimultaneousSlice Z(=nz), Coil(If SENSE recon included, =1; If SENSE recon not included, =nc), Time(=nt), SetOfSensitivityMaps].
if calc_rank
...
% Save solve_mtx in parameter structure p ONLY for real-time recon if p has no such field yet
if ~use_previous_solvemtx
    % Preallocate p.solve_mtx
    if include_sense_recon
        p.solve_mtx = zeros(ny*nz*nmap, nsamp*nc, datsz.x, datsz.ec, datsz.sl, nmsk, 'like',dat_mux); % Dim: [ny*nz*nmap, nsamp*nc, X, Echo, Slice, nmsk]
    else
        p.solve_mtx = zeros(ny*nz*nmap, nsamp, nc, datsz.x, datsz.ec, datsz.sl, nmsk, 'like',dat_mux); % Dim:[ny*nz*nmap, nsamp, nc, X, Echo, Slice, nmsk]
    end
    if p.use_GPU
        p.solve_mtx = gpuArray(p.solve_mtx);
    end
end

for msk_idx = 1 : nmsk
    if ~use_previous_solvemtx   % calculate ftyz_mtx only when solve_mtx needs to be calculated
    ...
    for echo = 1 : datsz.ec
        for slice = 1 : datsz.sl
            for x = 1 : datsz.x
                ...
                % Calculate solve_mtx if this is not saved in p
                if ~use_previous_solvemtx
                    ...
                    if include_sense_recon
                        ...
                        pixel_idx = any(gather(smap_mtx),2) ~= 0;
                        unknown_pixels = find(pixel_idx == 1); % Dim: [1, ny*nz*nmap]. 1 indicates a pixel is not in the background; 0 indicates a pixel is in the background and no sensitivity estimation exists for it.
                        ...
                        encode_mtx = encode_mtx(:, unknown_pixels) .* smap_mtx; % Dim: [nsamp*(1+extra_dat_vcc)*nc, num_unknown_pixels]
                    end

                    % Calculate the pseudo-inverse of the encoding matrices
                    if include_sense_recon
                        solve_mtx = get_solve_mtx(gather(encode_mtx), p.sense_lambda, p.sense_lambda_default_ratio); % Dim: [num_unknown_pixels, nsamp*(1+extra_dat_vcc)*nc]
                        if calc_rank
                            ranks(x, echo, slice, ONE, msk_idx) = rank(encode_mtx);
                        end
                        if p.RT_flag   % save solve_mtx if it's real-time recom
                            p.solve_mtx(:, :, x, echo, slice, msk_idx) = solve_mtx;
                        end
                    else
                        for coil = 1 : nc
                            solve_mtx = get_solve_mtx(gather(encode_mtx(:, :, coil)), p.sense_lambda, p.sense_lambda_default_ratio);
                            if calc_rank
                                ranks(x, echo, slice, coil, msk_idx) = rank(encode_mtx(:, :, coil));
                            end
                            if p.RT_flag   % save solve_mtx if it's real-time recom
                                p.solve_mtx(:, :, coil, x, echo, slice, msk_idx) = solve_mtx;
                            end
                        end
                    end
                    
                else   % use previously saved solve_mtx, will enter here only if ~p.cal_flag, i.e. accelerated time points in real-time recon
                    if ~p.use_GPU  
                        if include_sense_recon
                            solve_mtx = p.solve_mtx(:, :, x, echo, slice, msk_idx);
                        else
                            for coil = 1 : nc
                                solve_mtx = p.solve_mtx(:, :, coil, x, echo, slice, msk_idx);
                            end
                        end
                        if include_sense_recon  % calculate unknown_pixels, num_unknown_pixels
                            smap_mtx = smap(x, :, echo, slice, :, :, :); % Dim: [X(=1), Y(=ny), Echo(=1), Slice(=1), SimultaneousSliceZ(=nz), Coil(=nc), SetOfSensitivityMaps(=nmap)
                            smap_mtx = reshape(permute(smap_mtx, [6,2,5,7,1,3,4]), [1, nc, ny*nz*nmap]); % Dim: [1, nc, ny*nz*nmap]

                            unknown_pixels = find(any(gather(smap_mtx),2) ~= 0); % Dim: [1, ny*nz*nmap]. 1 indicates a pixel is not in the background; 0 indicates a pixel is in the background and no sensitivity estimation exists for it.
                            num_unknown_pixels = length(unknown_pixels);
                        end
                    end
                end

                if ~p.use_GPU  % calculate recon signal at each x when not using GPU
                    % Measured signal
                    ...
                    measured_sig = reshape(dat_mux(x, :, echo, slice, :, time_for_this_us_msk), [nsamp, nc, nt_to_recon]); % Dim: [nsamp, nc, nt_to_recon]
                    ...
                    if (nmsk == 1) && (nt_to_recon <= NT_THRESH) % Undersample scheme doesn't change with time(nt_to_recon=nt) AND nt_to_recon no larger than NT_THRESH, reconstruct all time points together
                        if include_sense_recon % If SENSE recon included
                            ...
                            for coil = 1 : nc
                                reconed_sig = solve_mtx * reshape(measured_sig(:, coil, :), [nsamp, nt_to_recon]); % Dim: [ny*nz, nt_to_recon]
                        ...
                        for time_idx = 1 : nt_to_recon
                            ...
                            if include_sense_recon % If SENSE recon included
                                ...
                                for coil = 1 : nc
                                    reconed_sig = solve_mtx * sig(:, coil);% Dim: [ny*nz, Time(=1)]
    ...
    if p.use_GPU
        ...
        nt_to_recon = length(time_for_this_us_msk); % Number of time points to reconstruct for this undersample mask. =nt if undersample scheme doesn't change with time; =1 if undersample scheme changes with time
        measured_sig = reshape(permute(gpuArray(dat_mux(:,:,:,:,:,time_for_this_us_msk)), [2,5,6,1,3,4]), [nsamp, nc, nt_to_recon, datsz.x, datsz.ec, datsz.sl]); % Dim: [nsamp, nc, nt_to_recon]
        ...
        if (nmsk == 1) && (nt_to_recon <= NT_THRESH) % Undersample scheme doesn't change with time(nt_to_recon=nt) AND nt_to_recon no larger than NT_THRESH, reconstruct all time points together
            if include_sense_recon % If SENSE recon included
                ...
                sig = reshape(measured_sig, [nsamp*(1+extra_dat_vcc)*nc, nt_to_recon, datsz.x, datsz.ec, datsz.sl]); % Dim: [nsamp*(1+extra_dat_vcc)*nc, nt_to_recon]
                sig = pagefun(@mtimes, p.solve_mtx(:,:,:,:,:,msk_idx), sig); % Dim: [ny*nz*nmap, nt_to_recon, datsz.x, datsz.ec, datsz.sl]
                sig = permute(reshape(sig, [ny, nz, nmap, nt_to_recon, datsz.x, datsz.ec, datsz.sl]), [5,1,6,7,2,8,4,3]); % Dim: [X(=1), Y(=ny), Echo(=1), Slice(=1), SimultaneousSlice Z(=nz), Coil(=1), Time(=nt_to_recon), SetOfSensitivityMap(=nmap)]
                dat_out(:, :, :, :, :, ONE, :, :) = gather(sig); % Dim: [X(=nx), Y(=ny), Echo(=nec), Slice(=nsl), SimultaneousSlice Z(=nz), Coil(=1), Time(=nt), SetOfSensitivityMap(=nmap)]
            else % If SENSE recon not included
                for coil = 1 : nc
                    sig = pagefun(@mtimes, p.solve_mtx(:,:,coil,:,:,:,msk_idx), reshape(measured_sig(:,coil,:,:,:,:), [nsamp, nt_to_recon, datsz.x, datsz.ec, datsz.sl])); % Dim: [ny*nz, nt_to_recon]
                    sig = permute(reshape(sig, [ny, nz, nt_to_recon]), [4,1,5,6,2,7,3]); % Dim: [X(=1), Y(=ny), Echo(=1), Slice(=1), SimultaneousSlice Z(=nz), Coil(=1), Time(=nt_to_recon)]
                    dat_out(:, :, :, :, :, coil, :) = gather(sig); % Dim: [X(=nx), Y(=ny), Echo(=nec), Slice(=nsl), SimultaneousSlice Z(=nz), Coil(=nc), Time(=nt)]
                end
            end
        else % Undersample scheme changes with time OR nt_to_recon larger than NT_THRESH
            for time_idx = 1 : nt_to_recon
                ...
                if include_sense_recon % If SENSE recon included
                    ...
                    sig = pagefun(@mtimes, p.solve_mtx(:,:,:,:,:,msk_idx), reshape(sig, [nsamp*(1+extra_dat_vcc)*nc, 1, datsz.x, datsz.ec, datsz.sl])); % Dim: [ny*nz*nmap, Time(=1)]
                    sig = permute(reshape(sig, [ny, nz, nmap, datsz.x, datsz.ec, datsz.sl]), [4,1,5,6,2,7,8,3]); % Dim: [X(=1), Y(=ny), Echo(=1), Slice(=1), SimultaneousSlice Z(=nz), Coil(=1), Time(=1), SetOfSensitivityMaps(=nmap)]
                    dat_out(:, :, :, :, :, ONE, time, :) = gather(sig); % Dim: [X(=nx), Y(=ny), Echo(=nec), Slice(=nsl), SimultaneousSlice Z(=nz), Coil(=1), Time(=nt), SetOfSensitivityMaps(=nmap)]
                else % If SENSE recon not included
                    for coil = 1 : nc
                        sig = pagefun(@mtimes, p.solve_mtx(:,:,coil,:,:,:,msk_idx), sig(:, coil, 1, :, :, :));% Dim: [ny*nz, Time(=1)]
                        sig = permute(reshape(sig, [ny, nz, datsz.x, datsz.ec, datsz.sl]), [3,1,4,5,2,6,7]); % Dim: [X(=1), Y(=ny), Echo(=1), Slice(=1), SimutlaneousSlice Z(=nz), Coil(=1), Time(=1)]
                        dat_out(:, :, :, :, :, coil, time) = gather(sig); % Dim: [X(=nx), Y(=ny), Echo(=nec), Slice(=nsl), SimultaneousSlice Z(=nz), Coil(=nc), Time(=nt)]
                    end % coil
                end % if include_sense_recon, else
            end % time
        end % if (nmsk == 1) && (nt <= NT_THRESH), else
    end % if p.use_GPU && use_previous_solvemtx
end % msk_idx
...
solve_mtx = (AtA + eye(size(AtA))*lambda) \ encode_mtx';               % Use pinv instead of inv so that no warning messages come up when cropped sensitivity maps are used
