function [reconed, sz_out, p] = mux_recon_1Dgrappa(ksp, p)
%
...
if p.debug && isfield(p, 'through_plane_ker') && ~isempty(p.through_plane_ker)
    fprintf('      Using previous through plane kernal...\n');
end
if p.caipi
...
reconed = zeros(datsz.x, datsz.y, datsz.ec, datsz.sl, p.mux, datsz.c, nt_solve, 'like',ksp);
...
if p.cal_flag
    ...
    % Save reconed ref data
    ref_dat = permute(fftshift(ref_dat,6), [1, 2, 3, 4, 6, 5, 7]);
    if strcmp(p.grappa_domain, 'imSpace')
        ref_dat = ifft2c(ref_dat);
    end
    ref_dat = reshape( ref_dat, [datsz.x, datsz.y, datsz.ec, datsz.sl*p.mux, ...
        datsz.c, p.num_mux_cycle]); % Dim: [FE, PE, Echo, Slice, Coil, TemporalPhase]

    % -- GRAPPA reconstruction.
    % Raw reference images.
    if p.caipi
        if p.cap_fov_shift > 0
            refic_raw = cat(p.PE_DIM, zeros(datsz.x, ny_zpad, datsz.ec, datsz.sl, datsz.c, p.mux, 'like',ksp), refic_raw);
    ...
    for slice = 1 : datsz.sl
        for echo = 1 : datsz.ec
            ...
            if p.caipi
                if strcmp(p.zpad_1Dgrappa, 'minZpad')    % Minimum zero padding
                    if p.cap_fov_shift > 0
                        refic = cat(p.PE_DIM, zeros(datsz.x, ny_zpad_residual_total, datsz.c, 'like',ksp), refic); % 2nd step to correct the shifts with fractional number of pixels.
                    else % p.cap_fov_shift < 0
                        refic = cat(p.PE_DIM, refic, zeros(datsz.x, ny_zpad_residual_total, datsz.c, 'like',ksp));
                    end
            ...
            p.through_plane_ker(:,:,:,:,echo,slice) = grappa_kernel(refic(acspos.x, acspos.y, :), ...
                [p.mux_kersz_1d_grappa.x, p.mux_kersz_1d_grappa.y], mux_R, p.grappa_domain, [datsz.x, datsz.y*mux_R]);
        end
    end
    if p.use_GPU
    ...
    for slice = 1 : datsz.sl
        for echo = 1 : datsz.ec        
            for tphase = 1 : nt_solve
                us_dat = zeros(datsz.x, datsz.y*mux_R, datsz.c, 'like',ksp);
                if p.cal_flag
                    us_dat(:, 1:mux_R:end, :) = reshape( ksp(:, :, echo, slice, :, p.mux*p.num_mux_cycle+tphase), [datsz.x, datsz.y, datsz.c]);        % Dim: [FE, PE, Coil]
                else
                    us_dat(:, 1:mux_R:end, :) = reshape( ksp(:, :, echo, slice, :, tphase), [datsz.x, datsz.y, datsz.c]);  
                end
                ...
                us_dat = grappa_recon(us_dat, p.through_plane_ker(:,:,:,:,echo,slice), p.grappa_domain, p.use_GPU); % Reconed data, Dim: [FE, PE, Coil]
                
                if p.use_GPU
                    us_dat = gpuArray(us_dat);
                end
                
                if p.caipi
                    if strcmp(p.grappa_domain, 'kSpace')
                        us_dat = ifft2c(us_dat);                    % Reconstructed coil images
                    end
                    
                    if strcmp(p.zpad_1Dgrappa, 'minZpad')% Minimum zero padding
                        if p.cap_fov_shift >0
                            us_dat = us_dat(:, ny_zpad_residual_total+1 : end, :); % Undo the effects of the 2nd step to correct the shifts with fractional number of pixels.
                        else % p.cap_fov_shift <0
                            us_dat = us_dat(:, 1 : end-ny_zpad_residual_total, :);
                        end
                        us_dat = reshape(us_dat, [datsz.x, datsz.y + ny_zpad, p.mux, datsz.c]);
                        if p.cap_fov_shift > 0
                            us_dat = flip(us_dat, 3);
                        end
                    else                                 % Use an easy way for zero padding
                        if p.cap_fov_shift > 0
                            us_dat = cat(p.PE_DIM, zeros(datsz.x, datsz.y, datsz.c, 'like',ksp), us_dat);
                        else % p.cap_fov_shift < 0
                            us_dat = cat(p.PE_DIM, cat(p.PE_DIM, us_dat(:, 1:datsz.y, :), zeros(datsz.x, datsz.y, datsz.c, 'like',ksp)), us_dat(:, datsz.y+1:end, :));
                        end
                        us_dat = reshape(us_dat, [datsz.x, 2*datsz.y, p.mux, datsz.c]);
                    end
                    
                    us_dat = fftshift(mux_shift(us_dat, 3, -slice_shift_amt, 'im'), 3);           % If minimum zero padding, this undos the effects of the 1st step to correct the shifts with fractional number of pixels.
                    
                    if p.cap_fov_shift > 0
                        reconed(:, :, echo, slice, :, :, tphase) = gather(us_dat(:, ny_zpad+1:end, :, :)); % reconed Dim: [FE, PE, Echo, Slice, Mux, Coil, TemporalPhase]
                    else % p.cap_fov_shift < 0
                        reconed(:, :, echo, slice, :, :, tphase) = gather(us_dat(:, 1:datsz.y, :, :));
                    end
                    if strcmp(p.grappa_domain, 'kSpace')
                    ...
                    if strcmp(p.grappa_domain, 'imSpace')
                        reconed(:, :, echo, slice, :, :, tphase) = ...                              % reconed Dim: [FE, PE, Echo, Slice, Mux, Coil, TemporalPhase]
                            gather(fftshift(reshape(us_dat, [datsz.x, datsz.y, p.mux, datsz.c]), 3));
                    else % p.grappa_domain is 'kSpace'
                        reconed(:, :, echo, slice, :, :, tphase) = ...
                            gather(fft2c( fftshift(reshape(ifft2c(us_dat), [datsz.x, datsz.y, p.mux, datsz.c]), 3) ));
                    end
                end
            end
        end
...
% sqrt(p.mux/mux_R): Adjust scaling in ifft2c to account for the upsampling. The scaling in ifft2c should be consistent with the scaling in fft2c; sqrt(p.mux): Account for the fact that the slice direction is not using orthonormal DFT encoding.
reconed = reshape( reconed/sqrt(mux_R), [datsz.x, datsz.y, datsz.ec, datsz.sl*p.mux, datsz.c, nt_solve]);                 % Dim: [FE, PE, Echo, Slice, Coil, TemporalPhase]

if p.cal_flag
    reconed = cat(p.T_DIM, ref_dat, reconed);
end
