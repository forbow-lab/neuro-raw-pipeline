...
%              p.sim_leakage            - True: Simulate signal leakage, i.e. use data from all slices in calibration and use data from slices specified in p.sim_leakage_keep_slices to synthesize accelerated data. Default: false.
%              p.sim_leakage_keep_slices- Slices to keep for synthesizing accelerated data when simulating signal leakage. Only used if p.sim_leakage is true. Defalut: 1.
...
if ~isfield(p, 'sim_leakage') || isempty(p.sim_leakage)
    p.sim_leakage = false;
end

if p.sim_leakage && (~isfield(p, 'sim_leakage_keep_slices') || isempty(p.sim_leakage_keep_slices))
    p.sim_leakage_keep_slices = 1;
end
...
if p.pmr_recon_sing
    if ~exist('im_sing', 'var') || isempty(im_sing)
        ...
        if p.partial_ky
            ...
            im_sing = homodyne(im_sing, nky, p.homodyne_ntran, p.homodyne_niter, 'kxky', 'xy', p.use_GPU);
        else
...
if p.pmr_sim_cal
    ...
    % -- Match partial k recon
    if p.partial_ky
        dat_cal_reps = homodyne(dat_cal_reps, nky, p.homodyne_ntran, p.homodyne_niter, 'xy', 'xy', p.use_GPU);
    end
...
% -- Keep only specified slices if simulating signal leakage
if p.sim_leakage
    dat_sing_slice_order = ifftshift(1 : nz);
    for sl = 1 : nz
        if ~ismember(sl, p.sim_leakage_keep_slices)
            dat_sing(:, :, :, :, :, dat_sing_slice_order == sl) = 0;
        end
    end
end
...
for msk_idx = 1 : nmsk
    ...
    dat_tseries(:, :, :, :, :, nkz*p.num_mux_cycle+1:end) = mux_dat(:, 1:nky, :, :, :, :);
    ...
    if p.sim_sense
        ...
        % Pseudo-multiple replica SNR
        [sense(msk_idx).signal, sense(msk_idx).replica] = separate_signal_and_replica(dat_rec(:, :, :, :, :, nkz*p.num_mux_cycle+1:end), NT_TO_SIM_SIGNAL);
    ...
    if p.sim_1d_grappa && (~p.use_gzblips || (p.use_gzblips && p.caipi)) % 1D GRAPPA only works with no-blip or CAIPI
        ...
        if p.debug
            trecon = toc(trecon);
            fprintf(' 1D GRAPPA Recon used %f s.\n', trecon);
        end
        
        % Pseudo-multiple replica SNR
        [oneD_grappa(msk_idx).signal, oneD_grappa(msk_idx).replica] = separate_signal_and_replica(dat_rec(:, :, :, :, :, nkz*p.num_mux_cycle+1:end), NT_TO_SIM_SIGNAL);
    ...
    if p.sim_slice_grappa
        ...
        if p.debug
            trecon = toc(trecon);
            fprintf(' Slice-GRAPPA Recon used %f s.\n', trecon);
        end
        
        % Pseudo-multiple replica SNR
        [slice_grappa(msk_idx).signal, slice_grappa(msk_idx).replica] = separate_signal_and_replica(dat_rec(:, :, :, :, :, nkz*p.num_mux_cycle+1:end), NT_TO_SIM_SIGNAL);
    ...
    if p.sim_split_slice_grappa
        ...
        if p.debug
            trecon = toc(trecon);
            fprintf(' Split-Slice-GRAPPA Recon used %f s.\n', trecon);
        end
        
        % Pseudo-multiple replica SNR
        [split_slice_grappa(msk_idx).signal, split_slice_grappa(msk_idx).replica] = separate_signal_and_replica(dat_rec(:, :, :, :, :, nkz*p.num_mux_cycle+1:end), NT_TO_SIM_SIGNAL);
