...
if isfield(p, 'pmr_noise_ref') && ~isempty(p.pmr_noise_ref)
    ...
    switch p.pmr_ref_type
        ...
        case 'mux1' % Use MUX1 acquisition as reference
            ...
            p_mux1.pmr_slices_ref = repmat(p_mux1.pmr_slices_ref(:), [nz, 1]); % Dim: [nsl*nz, 1]. Let the simulation reload the same data from ref pfile for Nyquist ghosting correction for each slice
