...
save_load_params = p.calc_snr_pmr || p.calc_sense_rsnr;
if save_load_params
    if p.cap_get_ecc
...
if p.md_ecc || (ismember(p.mux_recon_method, {'slice-grappa', 'split-slice-grappa'}) && p.slice_grappa_odd_even_fit) % Matrix-decoding ghost correction or (split-)slice-GRAPPA with odd even kernel fitting
    ...
    if p.vrgf
        ...
        switch p.ref_for_default_ecc
            ...
            case 'ref.h5'
                [dat, p] = default_ecc(dat, p, ref, slices); % Use ref.h5 file in default correction
            case 'ref_in_scan'
                [dat, p] = default_ecc(dat, p, p.pfile, slices); % Use the k-space data in the 1st pass in pfile for default correction
        end
    else
        if p.md_ecc
...
%                     or ref.h5 file name('*.h5'), or reference scan pfile name('*refscan.7').
...
%   p       - Parameter structure. See mux_epi_params.m for details.
%   ref     - Reference scan k-space data matrix(Dim: [Kx(=nx), Ky(=ny), Echo(=1), Slice(=nsl*mux), Coil(=nc)]),
%             or ref.h5 file name('*.h5'), or reference scan pfile name('*refscan.7'), or pfile name (Pxxxxx.7).
%   slices  - Slice indices(Normally order indices, not pfile indices) the
%             input 'dat' corresponds to. Only needed if the type of 'ref'
%             is ref.h5 file name or reference scan pfile name.
%
% Outputs
...
if strcmp(ref_type, 'ref.h5_file') || strcmp(ref_type, 'ref.dat_file') || strcmp(ref_type, 'ksp_data')
    datsz = get_dat_sz(dat, p);
end
if (~exist('slices', 'var') || isempty(slices)) && (strcmp(ref_type, 'ref.dat_file') || strcmp(ref_type, 'ref.h5_file') || strcmp(ref_type, 'ref_pfile'))
    error('No slice indices');
end

if p.cal_flag                           % Calculate/load pha_coe for all calibraion scans
    if strcmp(ref_type, 'ref.h5_file')  % Load coefficients from ref.h5 file 
        if p.debug
            fprintf('   Using ref.h5 file in default ghosting correction.\n');
        end
        pha_coe = rawload_refh5(ref, p.ny_pres, p.num_slices, p.pfile_header.ncoils, datsz.y, slices);
    
    elseif strcmp(ref_type, 'ref.dat_file')  % Load coefficients from ref.dat file 
        if p.debug
            fprintf('   Using ref.dat file in default ghosting correction.\n');
        end
        pha_coe = rawload_ref(datsz.y, p.num_slices, p.pfile_header.ncoils, p.frames, slices, p.coils, ref);
...
%
% Input
%   ref      - Reference scan k-space data, or '*.h5' file name, or '*refscan.7' file name, 
%              or Pxxxxx.7 (pfile name) when using integrated reference scan.
%
% Output
%   ref_type - What the input 'ref' contains.
%              'ksp_data'     - Reference scan k-space data.
%              'ref.dat_file' - Name of ref.dat file.
%              'ref.h5_file'  - Name of ref.h5 file.
...
if isnumeric(ref)
    ...
    if strcmp(ref(end-2:end), '.h5')
        ref_type = 'ref.h5_file';
    elseif strcmp(ref(end-3:end), '.dat')
        ref_type = 'ref.dat_file';
    else
        if strcmp(ref(end-1:end), '.7')
            ref_type = 'ref_pfile';
        else
            error('ref must be kspace data or *.h5, *.dat file or *.7 file.');
        end
    end
end
