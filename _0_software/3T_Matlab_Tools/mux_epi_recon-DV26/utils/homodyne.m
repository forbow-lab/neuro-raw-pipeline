function dat = homodyne(dat, ny_part, ntran, niter, in_dom, out_dom, use_GPU)
%
% function dat = homodyne(dat, ny_part, [ntran=2], [niter=4], [in_dom='KxKy'], [out_dom=in_dom], [use_GPU=false])
%
...
%   use_GPU - A flag to specify whether to use GPU for computation
%
% Output
...
% Use the for loop for offline recon (without GPU acceleration) because datsz.t >> 1 and will hit the RAM limit
if use_GPU 
    lowPassFilter = repmat( lowPassFilter,[datsz(1) 1 datsz(3:7)]);
    highPassFilter = repmat(highPassFilter,[datsz(1) 1 datsz(3:7)]);
    mergeWindow = repmat(mergeWindow,[datsz(1) 1 datsz(3:7)]);

    % Calculate high and low pass images in (x/Kx, Ky)
    % Get low pass views
    lowPass = dat .* lowPassFilter;
    % Get high pass views
    dat = dat .* highPassFilter;
