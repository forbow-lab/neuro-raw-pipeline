...
if ~exist('nt_to_recon_total', 'var') || isempty(nt_to_recon_total)
    nt_to_recon_total = p.num_passes - p.skip_ref_pass - p.mux_excited*p.cap_get_ecc - p.mux_encoded*p.num_mux_cycle;
end
if numel(nt_to_recon_total) ~= 1
    error('''nt_to_recon'' must be a scalar.');
end

p.nt_to_recon_total = min(p.num_passes - p.skip_ref_pass, nt_to_recon_total + p.mux_excited*p.cap_get_ecc + p.mux_encoded*p.num_mux_cycle);
...
if end_time_indx > p.num_passes - p.skip_ref_pass
...
p.tpoints_to_load = p.skip_ref_pass + (1 : end_time_indx);
p.first_scan_pass = p.skip_ref_pass + p.mux_excited*p.cap_get_ecc + p.mux_encoded*p.num_mux_cycle + 1;
