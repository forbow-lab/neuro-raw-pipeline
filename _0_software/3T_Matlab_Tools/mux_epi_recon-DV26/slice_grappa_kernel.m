...
dat_sing = dat_sing .* repmat(permute(us_msk.ftz_pha, [4,1,3,2]), [nx,1,nc,1]);
...
for ker_idx = 1 : nker
    if ~use_split % Standard slice-GRAPPA
        ...
        % -- Source matrix
        src_mtx = zeros(n_instances*nz, kersz(1)*kersz(2)*nc, 'like', dat_mux); % Dim: [n_instances*nz, kersz(1)*kersz(2)*nc]
        for z = 1 : nz
        ...
        tgt_fe_pos = floor(kersz(1)/2)+1;
        tgt_pe_pos = floor(kersz(2)/2)+1;
        tgt_mtx_raw = zeros(n_instances*nz, nc, nz, 'like', dat_mux); % Dim: [n_instances*nz, nc, nz]
        for z = 1 : nz
        ...
        % -- Raw kernel
        ker{ker_idx}.ker = zeros(kersz(1)*kersz(2)*nc, nc, nz, 'like', dat_mux);
        for z = 1 : nz
            tgt_mtx = tgt_mtx_raw(:, :, z); % Dim: [length(instances_to_keep), nc]
            AtA = src_mtx' * src_mtx;
            lambda = norm(AtA, 'fro') / size(src_mtx,2) * 0.02; % Regularization parameter, from Miki Lustig's code
            ker{ker_idx}.ker(:, :, z) = (AtA + eye(size(AtA))*lambda) \ (src_mtx' * tgt_mtx); % Dim: [kersz(1)*kersz(2)*nc, nc, nz]
        end
    end
    ...
    % -- Convolution kernel
    conv_ker = flipdim(flipdim(autocorr_ker, 1), 2); % Dim: [Kx(src), Ky(src), nc(src), nc(tgt), nz(tgt)]
...
mtx = zeros(prod(blk_sz(1:2)), n_instances, nc, nz, 'like', dat);
for z = 1 : nz
    for coil = 1 : nc
