...
if p.calc_snr_pmr || p.calc_sense_rsnr || p.cal_flag
    if ~use_previous_ccmtx % Will calculate coil compression matrices and save them into parameter structure
        ...
        p.ccmtx = zeros(p.num_coils, p.num_vcoils, nx_ccmtx, dsz.ec, dsz.sl, 'like',dat); % Dim: [Coil(=p.num_coils, Before compression), Coil(=p.num_vcoils, After compression), X(=1 if p.cc_method is 'single'; =dsz.x if p.cc_method is 'geometric'), Echo(=dsz.ec), Slice(=dsz.sl)]
    end
end
...
for echo = 1 : dsz.ec
    for slice = 1 : dsz.sl
        ...
        d = CC(d, fullsamp_dim, ccmtx, p); % Treat the time dimension as the Kz dimension when calling function CC.
