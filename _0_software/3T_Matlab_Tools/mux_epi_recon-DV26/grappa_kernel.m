...
clear acs_dat acs_mtx src_mtx tgt_mtx AtA
% -- Autocorrelation kernel
autocorr_ker = zeros(kersz(1), R*ceil(kersz(2)/2)*2-1, n_coil, n_coil, 'like',ker); % Dim: [FE(src), PE(src), n_coil(src), n_coil(tgt)]
for tgt_coil = 1 : n_coil
    for tgt_pe = 1 : R-1
        autocorr_ker( :, R-tgt_pe:R:R*kersz(2)-1, :, tgt_coil ) = ker(:, :, :, tgt_pe, tgt_coil);
    end
    autocorr_ker( floor(kersz(1)/2)+1, R*floor((kersz(2)-1)/2)+R, tgt_coil, tgt_coil) = 1;
end

% -- Convolution kernel
autocorr_ker = flip(flip(autocorr_ker, 1), 2); % Dim: [FE(src), PE(src), n_coil(src), n_coil(tgt)]

if strcmp(method, 'kSpace')
    ker = autocorr_ker;                              % Dim: [FE(src), PE(src), n_coil(src), n_coil(tgt)]
end

% -- Image space multiplication kernel
if strcmp(method, 'imSpace')
    ker = zpad(sqrt(kspsz(1)*kspsz(2))*autocorr_ker, [kspsz(1), kspsz(2), n_coil, n_coil]);
