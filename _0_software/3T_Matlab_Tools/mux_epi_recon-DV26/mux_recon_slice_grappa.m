...
if p.cal_flag
    if datsz.t == 0                                                          % If dat contains no accelerated time point then use synthesized mux data to calculate the grappa kernels
        ...
        dat_mux = p.mux_raw_data(:, :, :, :, :, end - abs(p.cap_fov_shift_cal)+1 : end); 
        if p.coil_compress
    ...
    for echo = 1 : datsz.ec
        for slice = 1 : datsz.sl
            ...
            if p.use_GPU
                for ker_idx = 1 : length(ker)
                    p.through_plane_ker{slice,echo,msk_idx}{ker_idx}.ker = gpuArray(p.through_plane_ker{slice,echo,msk_idx}{ker_idx}.ker);
                end
            end
        end
    end
...
if datsz.t > 0
    for echo = 1 : datsz.ec
        for slice = 1 : datsz.sl
            nker_per_slice = length(p.through_plane_ker{slice,echo,msk_idx});
            ...
            for ker_idx = 1 : nker_per_slice
                ker = p.through_plane_ker{slice,echo,msk_idx}{ker_idx}.ker;
                for time = 1 : datsz.t
                    dat_mux_for_recon = reshape(dat_mux(:, :, echo, slice , :, time), [datsz.x, ny_acq, datsz.c]); % Dim: [Kx(=nx), Ky(=ny_acq), Coil(=nc)]
                    tmp = slice_grappa_recon(dat_mux_for_recon, ker, us_msk, p.grappa_domain, p.use_GPU); % Dim: [Kx(=nx), Ky(=ny_acq), Coil(=nc), SimultaneousSlice Z(=nz)]
                    if nker_per_slice == 2 % Odd even kernel fitting
                        switch mod(p.through_plane_ker{slice,echo,msk_idx}{ker_idx}.ky_start, 2)
