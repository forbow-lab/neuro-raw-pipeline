function [d, snr_d] = mux_epi_main_offline(pfile, outfile, ext_cal, slices, nt_to_recon, n_vcoils, debug, recon_method, apply_fermi, use_homodyne, notch_thresh)
%
...
%   recon_method- '1Dgrappa' or numbers other than 1: use 1D-GRAPPA; Referenes 1. Blaimer M. et al. JMRI 2006; 24(2): 444-450. 2. Blaimer M. et al. MRM 2013; 69(4): 974-980.
%                 'sense' or number 1: use SENSE;
%                 'slice-grappa': use slice-GRAPPA; Reference: Kawin Setsompop, et al. MRM 2012;67(5):1210-24.
%                 'split-slice-grappa': use split-slice-GRAPPA. Reference: 1. Stephen F. Cauley, et al. MRM 2014;72(1):93-102. 2. Kawin Setsompop, et al. NeuroImage 2012;63:569-580.
...
if(~exist('debug', 'var'));        debug       = true;  end
...
if (~isnumeric(slices));           slices = str2num(slices);            end
if (~isnumeric(nt_to_recon));      nt_to_recon = str2num(nt_to_recon);  end
if (~isnumeric(n_vcoils));         n_vcoils = str2num(n_vcoils);        end
if (debug ~= 1)&&(debug ~= 0);     debug = str2num(debug);              end
if (~isnumeric(notch_thresh));     notch_thresh = str2num(notch_thresh);end
if (apply_fermi ~= 1)&&(apply_fermi ~= 0);     
    apply_fermi = str2num(apply_fermi);              
end
if (use_homodyne ~= 1)&&(use_homodyne ~= 0);     
    use_homodyne = str2num(use_homodyne);              
end
...
RT_flag = false;     % Flag for real-time recon. false: one-pass recon
...
p = mux_epi_params(pfile, slices, nt_to_recon, n_vcoils, debug, recon_method, apply_fermi, use_homodyne, notch_thresh, noise_fname, refp, RT_flag);
...
if p.md_ecc
    if strcmp(p.ref_for_md_ecc, 'ref pfile') && ~exist(refp, 'file') && p.cap_get_ecc
        ...
        if p.debug
            fprintf('\nNo reference scan pfile. Will use ECC data collected by mux sequence in matrix-decoding ghosting correction.\n\n');
        end
    end
...
if ~p.internal_cal && ~isnumeric(ext_cal)
    p_cal = mux_epi_params(ext_cal, slices, [], n_vcoils, debug, recon_method, apply_fermi, use_homodyne, notch_thresh, noise_fname_cal, refp_cal, RT_flag);
