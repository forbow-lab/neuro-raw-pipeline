function [dat, p] = sense_coil_comb(dat, p)
%
...
% Parameters
mux = p.mux;
cap_fov_shift_cal = p.cap_fov_shift_cal;
cap_fov_shift = p.cap_fov_shift;
p.mux = 1;
...
% Sensitivity maps
if p.cal_flag
    [smap, p] = sense_dat_cal_to_smap(dat(:, :, :, :, :, p.num_mux_cycle), p);
    if p.RT_flag
        p.smap = smap;
    end
else
...
% Combine coil images
[dat, p] = mux_recon_sense(dat, msk, p, smap, eddy); % Dim: [X(=nx), Y(=p.ny_pres), Echo(=nec), Slice(=nsl), Coil(=1), Time(=p.num_mux_cycle+nt), SetOfSensitivityMaps].
...
% Restore p parameters
p.mux = mux;
p.cap_fov_shift_cal = cap_fov_shift_cal;
p.cap_fov_shift = cap_fov_shift;
