function [dat, p] = mux_epi_process_data_slice_grappa(dat, p)
%
...
% Reconstruct calibration data in the first pass, otherwise create empty arrays
if p.cal_flag
...
if p.mux > 1
    ...
    if ~isfield(p, 'us_msk') || ~isempty(p.us_msk)
        if p.calc_snr_pmr && isfield(p, 'pmr_us_msk') && ~isempty(p.pmr_us_msk) % Enable the option to pass the undersampling mask via the parameter structure, so that this routine can be used for simulation purpose.
            ...
            p.us_msk = get_ky_omegaz_us_msk(p, datsz.y, size(dat, p.T_DIM), true); % Undersampling mask on the ky-omegaz plane, with inplane acceleration
        end
    end
    nmsk = length(p.us_msk);
    if nmsk > 1 % Undersampling pattern changes for each time point
        dat_reconed = zeros(datsz.x, datsz.y, datsz.ec, datsz.sl*p.mux, datsz.c, size(dat, p.T_DIM), 'like', dat); % Dim: [Kx(=datsz.x), Ky(=datsz.y), Echo(=datsz.ec), Slice(=datsz.sl*nz, z indices -floor(nz/2):1:(ceil(nz/2)-1)), Coil(=datsz.c), Time(=Number of accelerated mux acquisiitons)]
        for msk_idx = 1 : nmsk
            [dat_reconed(:, :, :, :, :, msk_idx), sz_out, p] = mux_recon_slice_grappa(dat(:, :, :, :, :, msk_idx), dat_cal, p.us_msk(msk_idx), p, msk_idx);
        end
        dat = dat_reconed;
        clear dat_reconed;
    else
        [dat, sz_out, p] = mux_recon_slice_grappa(dat, dat_cal, p.us_msk(1), p); % Dim: [Kx(=datsz.x), Ky(=datsz.y), Echo(=datsz.ec), Slice(=datsz.sl*nz, z indices -floor(nz/2):1:(ceil(nz/2)-1)), Coil(=datsz.c), Time(=Number of accelerated mux acquisiitons)]
    end
end
if ~isempty(dat_cal)
    ...
    for echo = 1 : datsz.ec
        for slice = 1 : datsz.sl*nz
            if p.cal_flag
                ...
                p.inplane_ker(:,:,:,:,slice,echo) = ker;
            end
                
            for time = 1 : size(dat, p.T_DIM)
                us_dat = reshape( dat(:, :, echo, slice, :, time), [datsz.x, datsz.y, datsz.c]); % Undersampled data, dim: [Kx, Ky, Coil]
                dat(:, :, echo, slice, :, time) = reshape(grappa_recon(us_dat, p.inplane_ker(:,:,:,:,slice,echo), p.grappa_domain, p.use_GPU), [datsz.x, datsz.y, 1, 1, datsz.c]);
            end
        end
    end
