function dat = sense_map_espirit(dat, nmap, ncalib, ksize, eigThresh_1, eigThresh_2, partial_ky, ny_part, debug, tmp_dir, use_GPU)
%
% function dat = sense_map_espirit(dat, [nmap=1], [ncalib=[20,20]], [ksize=[6,6]], [eigThresh_1=round(1.6*prod(ksize))(if using Matlab code) or 0.02(if using compiled C code)], 
%                                  [eigThresh_2=0.95], [partial_ky=false], [ny_part=[]], [debug=false], [tmp_dir='tmp_data/'], [use_GPU=false])
% 
...
if (ismac || isunix) && ~isempty(getenv('TOOLBOX_PATH')) && (ncalib(1) == ncalib(2)) && ~partial_ky % Use compiled C code
    ...
    [dat, ~] = dat2Kernel(dat, ksize);                                 % dat: k-space kernels (Use the same variable name to reduce memory usage). S: singular values.
    
    % Crop kernels and compute eigen-value decomposition in image space to get maps
    [dat, W] = kernelEig(dat(:, :, :, 1:eigThresh_1), [nx, ny], use_GPU);       % dat: sensitivity maps, Dim: [nx(UndersampledDim1), ny(UndersampledDim2), nc(Coils), nc(Set of maps)]. W: eigenvalues, Dim: [nx, ny, nc(Set of maps)]
