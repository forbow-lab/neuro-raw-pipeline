...
%                    [use_homodyne=true], [notch_thresh=0], [noise_fname], [refp], [RT_flag=false], [GPU_flag=false])
%
...
%   GPU_flag    - True: use GPU; False: not use GPU. Default: false
%
% Output:
...
if(~exist('GPU_flag', 'var'));                             GPU_flag = false;    end
...
%% DEBUG
if debug,
    fprintf('DEBUG: hdr.rdb_hdr.nslices=%d\n', hdr.rdb_hdr.nslices);
    fprintf('DEBUG: hdr.image.slquant=%d\n',   hdr.image.slquant);
end
...
if debug,
    fprintf(1,'DEBUG:  mux_epi_params.m ::: p.num_unmuxed_slices=%d, p.num_slices=%d, p.mux=%d\n',p.num_unmuxed_slices,p.num_slices,p.mux);
end
...
% Using GPU device
if GPU_flag && p.RT_flag && gpuDeviceCount > 0                % Use GPU only for real-time recon
    p.use_GPU = 1;
    if mod(slices, 2) == 1
        gpuDevice(1);
    else
