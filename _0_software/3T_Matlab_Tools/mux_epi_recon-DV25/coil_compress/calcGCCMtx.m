...
mtx = zeros(Nc,min(Nc,ws*Ny*Nz),Nx, 'like',calibDATA);
zpim = zpad(calibDATA,[Nx + ws-1,Ny,Nz,Nc]);
for n = 1:Nx
    [~,~,V] = svd(reshape(zpim(n:n+ws-1,:,:,:),ws*Ny*Nz,Nc), 'econ');
    mtx(:,:,n) = V;
end
