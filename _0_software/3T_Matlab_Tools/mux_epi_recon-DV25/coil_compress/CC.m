function [dat] = CC(DATA,dim,mtx,p)
% [res] = CC(DATA,dim,mtx)
%
...
if p.use_GPU
    DATA = permute(reshape(gpuArray(DATA), [Nx,Ny*Nz,Nc]), [2,3,1]);
    res = permute(reshape(pagefun(@mtimes, DATA, mtx), [Ny,Nz,ncc,Nx]),[4,1,2,3]);
else
    res = zeros(Nx,Ny,Nz,ncc, 'like',DATA);
    for n=1:Nx
        tmpc = reshape(DATA(n,:,:,:),[Ny*Nz,Nc]);
