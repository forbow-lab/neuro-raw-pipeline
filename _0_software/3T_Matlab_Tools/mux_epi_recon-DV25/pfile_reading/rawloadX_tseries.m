...
data = zeros(header.frsize,length(frames),length(echoes),length(slices),length(coils),length(passes), params.precision);
        ...
        while (sliceindex <= length(slices))
            ...
            while (echoindex <= length(echoes)) % -- Skip echoes before desired echo.
                ...
                while (frameindex <= length(frames))
                    ...
                    [fip,framecount,status] = skipsection(fip,framecount,frames(frameindex),framesize);
...
function [fileptr,counter,status] = skipsection(fileptr,counter,target,secsize)
