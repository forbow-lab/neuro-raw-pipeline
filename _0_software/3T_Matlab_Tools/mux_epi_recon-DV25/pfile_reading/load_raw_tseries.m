...
data = zeros(header.frsize, nframes, nechos, length(slices), ncoils, length(passes), params.precision);
...
if(rds_data_order)
    ...
    for(passindex = start_pass:length(passes))
        if(passes(passindex) < params.mux*params.num_mux_cycle) && (inplane_R > 1)
            ...
            for(shot = 0:inplane_R-1)
                for(sliceindex = 1:length(slices))
                    ...
                    if (params.slice_shuffling == 1)
                        slices_act = mod(slices - passes(passindex), params.num_slices);
                    end
            ...
            for(sliceindex = 1:length(slices))
                ...
                status = fseek(fip, rawhdrsize + acquired_pass*passbytes + slices_act(sliceindex)*slicebytes, -1);
                % Read the entire slice (all coils, all echos, all frames)
                [d,nread] = fread(fip, slicebytes/ptsize, dattypes{ptsize/2});
                if (nread == slicebytes/ptsize) && (status == 0)
                    % convert to complex
    ...
    if ~aborted_scan
        acquired_num_passes = passindex;
    end

    % -- Deal with partially acquired p-files.
    if params.cal_flag && acquired_num_passes < 1         % p-file contains no data.
        error('The p-file doesn''t contain any data.');
    end

    if params.cal_flag && acquired_num_passes < params.mux*params.num_mux_cycle % Data in p-file are all mux phase cycling frames.
        if isfield(params, 'ref_flag') && ~params.ref_flag
    ...
    params.nt_to_recon = acquired_num_passes + passes(1) - params.mux*params.num_mux_cycle;
    params.tpoints_to_load = passes(1) + (1 : min(params.num_passes, size(data,6)));
