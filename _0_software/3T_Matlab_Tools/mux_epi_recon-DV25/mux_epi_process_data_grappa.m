function [dat, p] = mux_epi_process_data_grappa(dat, p)
%
...
if p.use_gzblips && p.mux>1 && p.inplane_R>1 && p.cal_flag
    p.us_msk = get_ky_omegaz_us_msk(p, sz_dat.y, [], false);
end

% -- For inplane accelerated scans
if p.inplane_R > 1 && p.cal_flag
    ...
    if p.use_gzblips && p.mux>1                                          % Gz blip On acquisition
        ...
        dat_cal = zeros(sz_dat.x, sz_dat.y, sz_dat.ec, sz_dat.sl, sz_dat.c, 'like',dat);
        for idx = 1 : sz_dat.y
            dat_cal(:, idx, :, :, :) = dat_cal_raw(:, p.us_msk.ky(idx), :, :, :, p.us_msk.kz(idx)); % Synthesize a 2D slice-accelerated image that is fully sampled in-plane.
        end
...
if p.inplane_R > 1 && p.cal_flag
    ...
    switch p.grappa_domain
        case 'kSpace'
            p.inplane_ker = zeros(p.inplane_kersz.x, p.inplane_R*ceil(p.inplane_kersz.y/2)*2-1, ...
                sz_cal.c, sz_cal.c, sz_cal.ec, sz_cal.sl, sz_cal.t, 'like',dat);
        case 'imSpace'
            p.inplane_ker = zeros(sz_dat.x, sz_dat.y, sz_cal.c, sz_cal.c, sz_cal.ec, sz_cal.sl, sz_cal.t, 'like',dat);
    end
    ...
    if p.use_GPU
        p.inplane_ker = gpuArray(p.inplane_ker);
    end
end

clear dat_cal dat_cal_raw acs_dat
...
if p.inplane_R > 1
    ...
    for time = 1 : sz_dat.t
        if time <= p.mux_encoded*p.num_mux_cycle && p.cal_flag             % Skip inplane interpolation for the first 'p.mux_encoded*num_mux_cycle' time points.
            continue;
        end
        ...
        for slice = 1 : sz_dat.sl
            for echo = 1 : sz_dat.ec
                ...
                dat(:, :, echo, slice, :, time) = reshape( grappa_recon(us_dat, p.inplane_ker(:, :, :, :, echo, slice, FIRST),...
                    p.grappa_domain, p.use_GPU), [sz_dat.x, sz_dat.y, 1, 1, sz_dat.c]); % Currently sz_cal.t is always 1, so always use the FIRST time point.
...
if p.partial_ky
    dat = cat(p.PE_DIM, dat, zeros(sz_dat.x, ...
        p.ny_pres-sz_dat.y, sz_dat.ec, sz_dat.sl, sz_dat.c, sz_dat.t, 'like',dat));
...
if p.mux > 1
    ...
    [dat, sz_dat, p] = mux_recon_1Dgrappa(dat, p);                          % Dim:  [FE, PE, Echo, Slice, Coil, TemporalPhase]
...
if ~p.return_sos_im && p.sense_coil_comb                                 % Combine coil images using SENSE
    ...
    [dat, p] = sense_coil_comb(dat(:, 1 : ny_to_use, :, :, :, :), p);         % Dim: [X, Y(=p.ny_pres), Echo, Slice, Coil, Time]
    sz_dat.c = 1;
...
if p.partial_ky
    ...
    dat = homodyne(dat, ny_part, p.homodyne_ntran, p.homodyne_niter, 'xy', 'xy', p.use_GPU); % Data input to and output from homodyne are all image space data
end
